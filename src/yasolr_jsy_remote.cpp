...
static void processJSON(JsonDocument& doc) {
  ...
  switch (root["model"].as<uint16_t>()) {
    ...
    case MYCILA_JSY_MK_229: {
      if (grid.isUsing(Mycila::metric::Source::JSY_MK_163_REMOTE) ||
          grid.isUsing(Mycila::metric::Source::JSY_MK_227_REMOTE) ||
          grid.isUsing(Mycila::metric::Source::JSY_MK_229_REMOTE)) {
        updateGrid(root);
      } else {
        for (Mycila::Router::Output* output : {&output1, &output2}) {
          if (output->isUsing(Mycila::metric::Source::JSY_MK_163_REMOTE) ||
              output->isUsing(Mycila::metric::Source::JSY_MK_227_REMOTE) ||
              output->isUsing(Mycila::metric::Source::JSY_MK_229_REMOTE)) {
            updateOutput(output, root);
          }
        }
      }
      break;
    ...
    case MYCILA_JSY_MK_194: {
      ...
      if (root["channel1"].is<JsonObject>()) {
        if (grid.isUsing(Mycila::metric::Source::JSY_MK_193_CH1_REMOTE) ||
            grid.isUsing(Mycila::metric::Source::JSY_MK_194_CH1_REMOTE)) {
          updateGrid(root["channel1"].as<JsonObject>());
        } else {
          for (Mycila::Router::Output* output : {&output1, &output2}) {
            if (output->isUsing(Mycila::metric::Source::JSY_MK_193_CH1_REMOTE) ||
                output->isUsing(Mycila::metric::Source::JSY_MK_194_CH1_REMOTE)) {
              updateOutput(output, root["channel1"].as<JsonObject>());
            }
          }
        }
      }
      ...
      if (root["channel2"].is<JsonObject>()) {
        if (grid.isUsing(Mycila::metric::Source::JSY_MK_193_CH2_REMOTE) ||
            grid.isUsing(Mycila::metric::Source::JSY_MK_194_CH2_REMOTE)) {
          updateGrid(root["channel2"].as<JsonObject>());
        } else {
          for (Mycila::Router::Output* output : {&output1, &output2}) {
            if (output->isUsing(Mycila::metric::Source::JSY_MK_193_CH2_REMOTE) ||
                output->isUsing(Mycila::metric::Source::JSY_MK_194_CH2_REMOTE)) {
              updateOutput(output, root["channel2"].as<JsonObject>());
            }
          }
        }
      }
