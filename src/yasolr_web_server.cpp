...
Mycila::Task dashboardUpdateTask("Dashboard", []() {
  ...
  if (website.realTimePIDEnabled()) {
    website.updatePIDCharts(); // costly
  } else {
    website.updateWarnings(); // costly, do it only if real-time PID is disabled
  }
  dashboard.sendUpdates();
});
...
void rest_api() {
  ESP_LOGI(TAG, "Initialize REST API");
    ...
    .on(
      ...
      [](AsyncWebServerRequest* request, String filename, size_t index, uint8_t* data, size_t len, bool final) {
        if (!index) {
          ...
          if (partition) {
            ESP_LOGW(TAG, "Updating Safeboot recovery partition");
            ...
            if (esp_partition_erase_range(partition, 0, partition->size) == ESP_OK) {
              ESP_LOGI(TAG, "Safeboot partition formatted");
    ...
    .on(
      ...
      [](AsyncWebServerRequest* request) {
        ...
        if (!LittleFS.exists(YASOLR_MQTT_SERVER_CERT_FILE)) {
          ...
          ESP_LOGI(TAG, "Uploaded MQTT PEM server certificate:\n%s", serverCertFile.readString().c_str());
  ...
  webServer.on("/api/router", HTTP_GET, [](AsyncWebServerRequest* request) {
    ...
    router.getRoutedMetrics(routerMetrics, gridVoltage.value_or(NAN));
    ...
    for (const auto& output : router.getOutputs()) {
      ...
      Mycila::metric::Metrics outputMetrics;
      output->getRoutedMetrics(outputMetrics, gridVoltage.value_or(NAN));
