...
void YaSolR::WebsiteClass::_update(bool skipWebSocketPush) {
  ...
  _systemTempState.update(systemTemperatureSensor.isValid() ? systemTemperatureSensor.getTemperature() : 0);
  ...
  _output1Voltage.update(output1.getVoltage());
  ...
  _output2Voltage.update(output2.getVoltage());
