...
static void on_mqtt_grid_power(const std::string& topic, const std::string_view& payload);
static void on_mqtt_grid_voltage(const std::string& topic, const std::string_view& payload);

void yasolr_configure_mqtt_grid_source() {
  if (mqtt) {
...
static void publishData() {
  ...
  Mycila::metric::Metrics gridMetrics;
  ...
  {
    mqtt->publish((baseTopic + "/grid/apparent_power").c_str(), std::isnan(gridMetrics.apparentPower) ? "" : std::to_string(gridMetrics.apparentPower));
    mqtt->publish((baseTopic + "/grid/current").c_str(), std::isnan(gridMetrics.current) ? "" : std::to_string(gridMetrics.current));
    mqtt->publish((baseTopic + "/grid/energy").c_str(), gridMetrics.energy ? std::to_string(gridMetrics.energy) : "");
    mqtt->publish((baseTopic + "/grid/energy_returned").c_str(), gridMetrics.energyReturned ? std::to_string(gridMetrics.energyReturned) : "");
    mqtt->publish((baseTopic + "/grid/frequency").c_str(), std::isnan(gridMetrics.frequency) ? "" : std::to_string(gridMetrics.frequency));
    mqtt->publish((baseTopic + "/grid/online").c_str(), YASOLR_BOOL(grid.isConnected()));
    mqtt->publish((baseTopic + "/grid/power").c_str(), std::isnan(gridMetrics.power) ? "" : std::to_string(gridMetrics.power));
...

  {
    for (const auto& output : router.getOutputs()) {
      ...
      Mycila::metric::Metrics outputMeasurements;
      ...
      mqtt->publish((outputTopic + "/energy").c_str(), outputMeasurements.energy ? std::to_string(outputMeasurements.energy) : "");
...
static void haDiscovery() {
  ...
  haDiscovery->publish<Mycila::HA::Update>("firmware_update", "Firmware Update", YASOLR_RELEASE_URL, "~/system/app/version", "~/system/app/latest_version");
  ...
  haDiscovery->publish<Mycila::HA::Gauge>("remote_jsy_msg_rate", "Remote JSY Message Rate", "~/system/remote_jsy/msg_rate", nullptr, nullptr, "msg/s", Mycila::HA::Category::DIAGNOSTIC);
  yield();
  ...
  haDiscovery->publish<Mycila::HA::Counter>("output1_energy", "Output 1 Energy", "~/router/output1/energy", "energy", nullptr, "Wh");
  haDiscovery->publish<Mycila::HA::Gauge>("output1_power", "Output 1 Power", "~/router/output1/power", "power", nullptr, "W");
  haDiscovery->publish<Mycila::HA::Gauge>("output1_power_factor", "Output 1 Power Factor", "~/router/output1/power_factor", "power_factor");
  yield();
  ...
  haDiscovery->publish<Mycila::HA::Counter>("output2_energy", "Output 2 Energy", "~/router/output2/energy", "energy", nullptr, "Wh");
  haDiscovery->publish<Mycila::HA::Gauge>("output2_power", "Output 2 Power", "~/router/output2/power", "power", nullptr, "W");
  haDiscovery->publish<Mycila::HA::Gauge>("output2_power_factor", "Output 2 Power Factor", "~/router/output2/power_factor", "power_factor");
  yield();
