...
String Mycila::OTAConfigClass::getID() const { return AppInfo.firmware; }
String Mycila::OTAConfigClass::getTitle() const { return AppInfo.name + " Web Updater"; }
String Mycila::OTAConfigClass::getFWVersion() const { return AppInfo.version; }
...
bool Mycila::DisplayConfigClass::isEnabled() const { return Mycila::Config.getBool(KEY_DISPLAY_ENABLE); }
Mycila::DisplayType Mycila::DisplayConfigClass::getType() const {
  const String val = Mycila::Config.get(KEY_DISPLAY_TYPE);
  if (val == "SSD1306")
    return Mycila::DisplayType::SSD1306;
  if (val == "SH1107")
    return Mycila::DisplayType::SH1107;
  return Mycila::DisplayType::SH1106;
}
int32_t Mycila::DisplayConfigClass::getClockPin() const { return Mycila::Config.get(KEY_DISPLAY_CLOCK_PIN).toInt(); }
int32_t Mycila::DisplayConfigClass::getDataPin() const { return Mycila::Config.get(KEY_DISPLAY_DATA_PIN).toInt(); }
uint32_t Mycila::DisplayConfigClass::getRotation() const {
  const uint32_t val = Mycila::Config.get(KEY_DISPLAY_ROTATION).toInt();
  return val == 90 || val == 180 || val == 270 ? val : 0;
}
uint32_t Mycila::DisplayConfigClass::getPowerSaveDelay() const { return Mycila::Config.get(KEY_DISPLAY_POWER_SAVE_DELAY).toInt(); }
