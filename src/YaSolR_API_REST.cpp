...
void YaSolR::YaSolRClass::_initREST() {
  ...
  Mycila::HTTPd.apiPOST("/relays/" NAME_RELAY2, [this](AsyncWebServerRequest* request) {
    if (relay2.isEnabled() && request->hasParam("state", true)) {
      ...
      uint32_t duration = request->hasParam("duration", true) ? request->getParam("duration", true)->value().toInt() : 0;
      if (state == YASOLR_ON)
        Mycila::RelayManager.tryRelayState(NAME_RELAY2, true, duration);
      else if (state == YASOLR_OFF)
        Mycila::RelayManager.tryRelayState(NAME_RELAY2, false, duration);
    }
    request->send(200);
  });
  ...
  Mycila::HTTPd.apiGET("/debug", [this](AsyncWebServerRequest* request) {
    ...
    output1PZEM.toJson(root["pzem"][0].to<JsonObject>());
    output2PZEM.toJson(root["pzem"][1].to<JsonObject>());
    Mycila::TaskMonitor.toJson(root["stack"].to<JsonObject>());
    loopTaskManager.toJson(root["taskManagers"][0].to<JsonObject>());
    meterTaskManager.toJson(root["taskManagers"][1].to<JsonObject>());
    routerTaskManager.toJson(root["taskManagers"][2].to<JsonObject>());
    response->setLength();
    request->send(response);
  });
}
