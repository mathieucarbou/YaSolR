...
// pro mode
  ...
  _jsyReset.attachCallback([] PUSH_BUTTON_CARD_CB { Mycila::JSY.resetEnergy(); });
...
void YaSolR::Website::update() {
  _update(false);
}

void YaSolR::Website::_update(bool skipWebSocketPush) {
  ...
  _output1DimmerSlider.update(static_cast<int>(Mycila::Router.outputs[0].dimmer.getLevel()));
  ...
  _output2DimmerSlider.update(static_cast<int>(Mycila::Router.outputs[1].dimmer.getLevel()));
  ...
  Mycila::Logger.debug(TAG, "Updated in %u ms", (micros() - start) / 1000);
}
...
void YaSolR::Website::_outputDimmerSlider(Card* card, int idx) {
  card->attachCallback([card, idx, this](int value) { // 0-100
    ...
    card->update(static_cast<int>(Mycila::Router.outputs[idx].dimmer.getLevel()));
    _dashboard.refreshCard(card);
  });
}
