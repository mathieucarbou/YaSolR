...
Mycila::Task profilerTask("loopTaskManager.log()", [](void* params) { loopTaskManager.log(); });
...
Mycila::Task configureJSYTask("configureJSYTask", [](void* params) {
  ...
  if (Mycila::Config.getBool(KEY_JSY_ENABLE)) {
    jsy.begin(Mycila::Config.get(KEY_JSY_RX_PIN).toInt(), Mycila::Config.get(KEY_JSY_TX_PIN).toInt(), &MYCILA_JSY_SERIAL, true);
    if (jsy.isEnabled()) {
...
Mycila::Task configureDebugTask("configureDebugTask", [](void* params) {
  ...
  if (Mycila::Config.getBool(KEY_DEBUG_ENABLE)) {
    Mycila::Logger.info(TAG, "Enabling profiling for all tasks");
...
void YaSolR::YaSolRClass::_initTasks() {
  // TODO: ⚠️ profiler with json output, status json output of task status (enable / disabled / last time / last elapsed, etc.) (like coroutine lib: https://github.com/bxparks/AceRoutine/)
  // TODO: ⚠️ move TaskManager to oss
