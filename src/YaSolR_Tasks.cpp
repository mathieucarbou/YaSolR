...
Mycila::Task mqttTask("MQTT.loop()", [](void* params) { Mycila::MQTT.loop(); });
...
Mycila::Task configureMQTTTask("configureMQTTTask", [](void* params) {
  // TODO: new mqtt impl from esp (#include "mqtt_client.h") - see lilygo eth sample: https://github.com/Xinyuan-LilyGO/LilyGO-T-ETH-Series/blob/master/examples/ESPMQTTSClient/ESPMQTTSClient.ino
...
void YaSolR::YaSolRClass::_initTasks() {
  ...
  mqttTask.setType(Mycila::TaskType::FOREVER);
  mqttTask.setManager(&loopTaskManager);
  mqttTask.setEnabledWhen([]() { return Mycila::MQTT.isEnabled(); });

  publisherTask.setType(Mycila::TaskType::FOREVER);
  publisherTask.setManager(&ioTaskManager);
  ...
  haDiscoTask.setManager(&ioTaskManager);
