...
// pro mode
  ...
  _stateGridPowerMQTT.setTab(&_stateTab);
...
// &webServer, YASOLR_ADMIN_USERNAME, , , , ,
#ifdef APP_VERSION_PRO
  ...
  _firmName.set((Mycila::AppInfo.name + " " + Mycila::AppInfo.model).c_str());
...
void YaSolR::WebsiteClass::_relaySwitch(Card* card, Mycila::Relay* relay) {
  card->attachCallback([card, relay, this](int value) {
    if (relay->isEnabled()) {
      Mycila::RelayManager.tryRelayState(relay, value);
    }
