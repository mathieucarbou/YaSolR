...
function callDimmer(dimmer_name, dimmer, cb) {
  ...
  if (dimmerPercentage > 1) {
    ...
    if (Date.now() - dimmer.lastActivation >= CONFIG.DIMMER_TURN_OFF_DELAY) {
      dimmerPercentage = 0;
      relay = "&on=false";
      if (CONFIG.DEBUG)
        print(scriptName, ":", "Turning off dimmer relay: ", dimmer_name);
    } else {
      dimmerPercentage = 1;
    }
  }
  let dimmer_ip = CONFIG.DIMMERS[dimmer_name].IP;
  // Build the URL to call...
  const url = "http://" + dimmer_ip + "/rpc/Light.Set?id=0" + relay + "&brightness=" + dimmerPercentage + "&transition_duration=0.5";
...
function callDimmers(cb) {
  ...
  for (let dimmer_name in DIVERT.dimmers) {
    dimmer = DIVERT.dimmers[dimmer_name];
...
function divert(gridVoltage, gridPower) {
  ...
  for (let dimmer_name in DIVERT.dimmers) {
    ...
    if (CONFIG.DEBUG > 0)
      print(scriptName, ":", "Dimmer:", dimmer_name, " => ", dimmer.powerToDivert, "W,", dimmer.dutyCycle, "%", "PID mode: ", PID.mode);
  }
...
function getDynUserSettings() {
  for (let user_def in usercomponents) {
    setByPath(CONFIG, user_def, Virtual.getHandle(usercomponents[user_def]).getValue());
  }
}
