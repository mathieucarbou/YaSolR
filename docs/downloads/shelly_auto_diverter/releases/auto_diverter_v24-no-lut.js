...
function callDimmer(dimmer_name, cb) {
  ...
  if (DIVERT.dimmers[dimmer_name].standby) {
    ...
    // normal routing mode
    // calculate dimmer percentage based on duty cycle
    dimmerPercentage = Math.round(constrain(DIVERT.dimmers[dimmer_name].dutyCycle * 100, CONFIG.DIMMERS[dimmer_name].MIN, CONFIG.DIMMERS[dimmer_name].MAX));
