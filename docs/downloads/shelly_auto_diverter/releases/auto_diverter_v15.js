...
const CONFIG = {
  ...
  GRID_SOURCE: {
    ...
    // Set the grid mode here. It will be used to call the correct Shelly API.
    // Values: "SINGLE" or "TRI"
    PHASES: "SINGLE",
  ...
  DIMMERS: {
    boiler: {
      ...
      // When set to true, if you activate remotely the bypass to force a heating, then the script will detect it and turn this dimmer off
      ...
      // Minimum value to set on the dimmer to have no power sent to the resistive load.
      // When the dimmer reaches this MIN value, the load should not be powered anymore.
      // If the dimmer stays at this MIN value for DIMMER_TURN_OFF_DELAY milliseconds, then the dimmer will be turned off.
      // Adjust this value according to your voltage regulator needs for lower voltage
      // Range: 1-99
      MIN: 1,
      // Maximum value to set on the dimmer to have some power sent to the resistive load.
      // Adjust this value according to your voltage regulator needs for higher voltage
      // Range: 1-99
      MAX: 99,
    },
    // pool_heater: {
...
const usercomponents = {
  ...
  // "DIMMERS.boiler.EXCESS_POWER_LIMIT": "number:201",
  // "DIMMERS.pool_heater.EXCESS_POWER_LIMIT": "number:202",
  // etc
}
...
function validateConfig(cb) {
  ...
  if (CONFIG.GRID_SOURCE.PHASES !== "SINGLE" && CONFIG.GRID_SOURCE.PHASES !== "TRI") {
    print(scriptName, ":", 'ERR: GRID PHASES must be "SINGLE" or "TRI"');
    return;
  }
  ...
  if (dimmer.standby) {
    ...
    if (dimmerPercentage > CONFIG.DIMMERS[dimmer_name].MIN) {
      ...
      if (Date.now() - dimmer.lastActivation >= CONFIG.DIMMERS[dimmer_name].DIMMER_TURN_OFF_DELAY) {
        ...
        relay = "&on=true";
        if (CONFIG.DEBUG > 1)
          console.log(scriptName, ":", "[" + dimmer_name + "] ROUTING => relay: ON, dimmer:", dimmerPercentage + "% (waiting to turn relay off)");
      }
    }
  }
...
function onSwitchGetStatus(result, errCode, errMessage, data) {
  ...
  for (let dimmer_name in DIVERT.dimmers) {
    ...
    if (CONFIG.DIMMERS[dimmer_name].BYPASS_CONTROLLED_BY_EM) {
      if (result.output) {
        console.log(scriptName, ":", "[" + dimmer_name + "] Bypass ON => turning off");
...
function divert(gridVoltage, gridPower) {
  ...
  for (let dimmer_name in DIVERT.dimmers) {
    ...
    if (dimmer.fullPower) dimmer.powerToDivert = dimmer.maxPower;
    if (dimmer.standby) dimmer.powerToDivert = 0;
...
function readShellyEM() {
  ...
  // EM1.GetStatus: for Shelly EM
  // EM.GetStatus: for Shelly 3EM
  const emMethod = CONFIG.GRID_SOURCE.PHASES === "SINGLE" ? "EM1.GetStatus" : "EM.GetStatus";
  Shelly.call(emMethod, { id: 0 }, function (result, errCode, errMessage) {
    ...
    if (CONFIG.GRID_SOURCE.PHASES === "SINGLE") {
      DIVERT.gridVoltage = result.voltage;
      DIVERT.gridPower = result.act_power;
    } else if (CONFIG.GRID_SOURCE.PHASES === "TRI") {
