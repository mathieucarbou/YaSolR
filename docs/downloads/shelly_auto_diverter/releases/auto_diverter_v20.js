...
function callDimmer(dimmer_name, dimmer, cb) {
  ...
  Shelly.call("HTTP.GET", { url: url, timeout: 5 }, function (result, errCode, errMessage, data) {
    if (errCode) {
      print(scriptName, ":", "[HTTP.GET] ERR:", errCode, errMessage);
      data.dimmer.rpc = "failed";

    } else if (result.code !== 200) {
      const rpcResult = JSON.parse(result.body);
      print(scriptName, ":", "[HTTP.GET] ERR:", rpcResult.code, rpcResult.message);
      data.dimmer.rpc = "failed";
...
function enableBypass(dimmer_name, dimmer) {
  if (dimmer.bypass) {
...
function disableBypass(dimmer_name, dimmer) {
  if (!dimmer.bypass) {
...
function enableDivert(dimmer_name, dimmer) {
  dimmer.standby = false;
  dimmer.bypass = false;
  print(scriptName, ":", "[" + dimmer_name + "]", "=> Dimmer AUTO");
}

function disableDivert(dimmer_name, dimmer) {
  dimmer.standby = true;
...
function switchGetStatus() {
  ...
  Shelly.call("Switch.GetStatus", { id: DIVERT.switchID }, function onSwitchGetStatus(result, errCode, errMessage) {
    ...
    for (let dimmer_name in DIVERT.dimmers) {
      let dimmer = DIVERT.dimmers[dimmer_name];
      if (result.output) {
        enableBypass(dimmer_name, dimmer);
      } else {
        disableBypass(dimmer_name, dimmer);
      }
    }
