...
const CONFIG = {
  ...
  PID: {
    ...
    // Useful when the relationship between setpoint and required output is partially known.
    FEED_FORWARD: 0,
    // Input filter coefficient (alpha) for first-order lag filter (exponential smoothing).
    // Filtered = alpha * input + (1 - alpha) * lastInput
    // 1.0 = no filtering (raw input, default), 0.0 = maximum filtering.
    ...
    // For JSY at ~330ms intervals, try 0.3 to 0.5
    FILTER_ALPHA: 1.0,
    // Time sampling: if true, integral and derivative gains are adjusted for elapsed
    // time between calls, keeping accuracy when call intervals vary.
    TIME_SAMPLING: false,
    // Target Grid Power (W)
    SETPOINT: -100,
    // Output Minimum (W): lower bound on the PID output.
    // Must be sufficiently negative to handle the largest sudden load that can turn on
    // (e.g. an EV charger starting at 7 kW).
    // With back-calculation anti-windup: iTerm = output - pTerm - dTerm - FF.
    // For iTerm to actually wind down, |OUT_MIN| must exceed the pTerm contribution:
    //   pTerm (error mode) = Kp * load_power  (e.g. 0.1 * 7000 = 700 W)
    // => OUT_MIN must be MORE negative than -Kp * max_sudden_load, otherwise the
    //    pTerm alone fills the budget and iTerm stays positive => overshoot at load-off.
    // Recommended: OUT_MIN <= -(max_sudden_load_power), e.g. -7000 for a 7 kW EV charger.
    OUT_MIN: -800,
...
let PID = {
  // PID Input (after filter)
  input: null,
  // PID Output
  output: 0,
  // Last error value (null = not yet computed, so dError = 0 on first call)
  error: null,
  // Proportional Term
  pTerm: 0,
  // Integral Term
  ...
  // Derivative Term
  dTerm: 0,
  // Timestamp of last compute() call in ms (null = not yet called)
  lastTime: null,
};
...
function calculatePID(rawInput) {
  const now = Date.now();

  // Compute elapsed time in seconds since last call (0 on first call)
  const dt = (PID.lastTime !== null) ? (now - PID.lastTime) / 1000.0 : 0.0;

  // Apply first-order lag filter (exponential smoothing) to input.
  // Filtered = alpha * raw + (1 - alpha) * lastFiltered
  // alpha = 1.0: no filtering (default); alpha → 0: heavy smoothing.
  let input;
  if (PID.input === null) {
    input = rawInput;
    PID.input = rawInput;
  } else {
    input = CONFIG.PID.FILTER_ALPHA * rawInput + (1.0 - CONFIG.PID.FILTER_ALPHA) * PID.input;
  }

  // Error and deltas
  const error = CONFIG.PID.SETPOINT - input;
  // dError = 0 on first call (PID.error is null) to avoid a large initial derivative kick
  const dError = (PID.error !== null) ? error - PID.error : 0;
  const dInput = input - PID.input; // Note: if setpoint never changes, dInput = -dError

  // Adjust integral/derivative gains for elapsed time when time-sampling is enabled
  let ki = PID.ki;
  let kd = PID.kd;
  if (CONFIG.PID.TIME_SAMPLING && dt > 0) {
    ki = ki * dt;
    kd = kd / dt;
  }
  ...
  if (CONFIG.PID.P_MODE === "error") {
    // Traditional: instantaneous response to current error
    PID.pTerm = PID.kp * error;
  } else { // "input": accumulate proportional term from input changes (avoids setpoint-change kick)
    PID.pTerm += PID.kp * dInput;
  }
  ...
  if (CONFIG.PID.D_MODE === "input") {
    // Derivative on measurement: avoids derivative kick when setpoint changes suddenly
    PID.dTerm = -kd * dInput;
  } else { // "error": traditional derivative on error
    PID.dTerm = kd * dError;
  }

  // Calculate output and clamp to output limits
  PID.output = constrain(PID.pTerm + PID.iTerm + PID.dTerm + CONFIG.PID.FEED_FORWARD, CONFIG.PID.OUT_MIN, CONFIG.PID.OUT_MAX);

  // Integral anti-windup: back-calculation (same as MycilaPID CLAMP mode).
  // After clamping, back-calculate iTerm so it reflects only what fits within limits.
  // This prevents iTerm from winding up beyond what is needed to stay within output bounds.
  PID.iTerm = PID.output - PID.pTerm - PID.dTerm - CONFIG.PID.FEED_FORWARD;

  if (CONFIG.DEBUG > 0) {
    print(scriptName, ":", "[PID] Mode:", PID.mode, "| Input:", input, "W | Error:", error, "W | pTerm:", PID.pTerm, "W | iTerm:", PID.iTerm, "W | dTerm:", PID.dTerm, "W | FF:", CONFIG.PID.FEED_FORWARD, "W | Output:", PID.output, "W");
  }
...
function callDimmer(dimmer_name) {
...
function divert(gridVoltage, gridPower) {
  ...
  if (Math.abs(gridPower - CONFIG.PID.SETPOINT) < CONFIG.PID.HIGH_LOW_SWITCH) {
    PID.mode = "LOW";
    ...
    if (CONFIG.DEBUG > 1)
      print(scriptName, ":", "[divert] Using LOW PID parameters: Kp=", PID.kp, " Ki=", PID.ki, " Kd=", PID.kd);
    ...
    if (CONFIG.DEBUG > 1)
      print(scriptName, ":", "[divert] Using HIGH PID parameters: Kp=", PID.kp, " Ki=", PID.ki, " Kd=", PID.kd);
  }
  ...
  for (let dimmer_name in CONFIG.DIMMERS) {
    ...
    if (DIVERT.dimmers[dimmer_name].bypass) {
      ...
      DIVERT.dimmers[dimmer_name].powerToDivert = Math.round((availablePowerToDivert - routedPower) * CONFIG.DIMMERS[dimmer_name].POWER_RATIO);
      ...
      routedPower += DIVERT.dimmers[dimmer_name].powerToDivert;
    }
  ...
  // Reset PID when not routing but we have excess power (gridPower < setpoint).
  // This avoids iTerm accumulating to PID max, which would cause a big diverting spike when routing resumes.
  // We do NOT reset when gridPower > setpoint to allow iTerm to accumulate toward OUT_MIN,
  // which helps absorb big load changes (e.g. EV charging that suddenly stops).
  // See: https://github.com/mathieucarbou/YaSolR/discussions/158
  // Mirrors pidController.reset(0) in yasolr_pid.cpp:
  //   - keeps PID.input (filter state preserved, like C++ _lastInput)
  //   - resets PID.error to null (like C++ _lastError = NAN, so dError = 0 on next call)
  //   - resets PID.lastTime to null (like C++ _lastTime = 0, so dt = 0 on next call)
  //   - sets iTerm = output = 0 (like C++ _iTerm = _lastOutput = clamp(0))
  if (routedPower === 0 && gridPower < CONFIG.PID.SETPOINT) {
    PID.pTerm = 0;
    PID.iTerm = 0;
    PID.dTerm = 0;
    PID.output = 0;
  ...
  if (CONFIG.BYPASS === "api") {
    ...
    Shelly.call("Switch.GetStatus", { id: DIVERT.switchID }, function (result, errCode, errMessage) {
      if (errCode === 404) {
...
function validateConfig(cb) {
  ...
  if (CONFIG.PID.P_MODE !== "error" && CONFIG.PID.P_MODE !== "input") {
    print(scriptName, ":", "ERR: PID.P_MODE must be 'error' or 'input'");
    return;
  }

  if (CONFIG.PID.D_MODE !== "error" && CONFIG.PID.D_MODE !== "input") {
    print(scriptName, ":", "ERR: PID.D_MODE must be 'error' or 'input'");
    return;
  }

  if (CONFIG.PID.FILTER_ALPHA < 0 || CONFIG.PID.FILTER_ALPHA > 1) {
    print(scriptName, ":", "ERR: PID.FILTER_ALPHA must be between 0 and 1");
    return;
  }
...
function shellyDivert() {
  if (DIVERT.remainingDimmers) {
    return;
  }

  DIVERT.remainingDimmers = Object.keys(CONFIG.DIMMERS).length;
  if (CONFIG.DEBUG > 1) {
    print(scriptName, ":", "shellyDivert() - START");
  }

  const emMethod = CONFIG.GRID_SOURCE.TYPE === "3EM" ? "EM.GetStatus" : "EM1.GetStatus";
  Shelly.call(emMethod, { id: 0 }, function (result, errCode, errMessage, data) {
    if (errCode) {
...
validateConfig(function () {
  ...
  getDeviceInfo(function () {
    ...
    if (CONFIG.GRID_SOURCE.TYPE === "EM") {
      ...
      Timer.set(200, true, shellyDivert);
