...
function callDimmer(dimmer_name, cb) {
  ...
  if (dimmer.rpc === "pending") {
    // previous call still pending, skip this one
    if (CONFIG.DEBUG > 1)
      print(scriptName, ":", "[" + dimmer_name + "] => Skipping call: previous call still pending");
    return;
  }
  ...
  Shelly.call("HTTP.GET", { url: url, timeout: 5 }, function (result, errCode, errMessage, data) {
    if (errCode) {
      ...
      data.dimmer.url = ""; // allow the same request to be retried next time
...
function callDimmers(cb) {
  ...
  function processDimmers(cb) {
    for (let dimmer_name in DIVERT.dimmers) {
      ...
      // call dimmer
      if (CONFIG.DEBUG > 1)
        print(scriptName, ":", "[" + dimmer_name + "] Calling... ");

      callDimmer(dimmer_name, function recallMe() {
        processDimmers(cb)
      });
...
function switchGetStatus(cb) {
  ...
  Shelly.call("Switch.GetStatus", { id: DIVERT.switchID }, function onSwitchGetStatus(result, errCode, errMessage, data) {
    if (errCode === 404) {
...
function shellyDivert(cb) {
  ...
  Shelly.call(emMethod, { id: 0 }, function (result, errCode, errMessage, data) {
    ...
    divert(DIVERT.gridVoltage, DIVERT.gridPower, data.cb);
  }, { emMethod: emMethod, cb: cb });
}
