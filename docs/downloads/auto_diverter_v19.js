 ...
 * - v19: Reduce call count to avoid Uncaught Error: Too many calls in progress
 *        Changed config for BYPASS mode and clarified its usage and operation
 *        Clarified doc for GRID_SOURCE.TYPE
 *        Fix MAX validation
 *        Fix MIN-MAX ranges
 *        Added /script/1/status?reset=1 to reset the states
 *        Added /script/1/status?setpoint=<value> to set PID setpoint on the fly
 * ======================================
 */
...
function validateConfig(cb) {
  ...
  for (let dimmer_name in CONFIG.DIMMERS) {
    ...
    if (CONFIG.DIMMERS[dimmer_name].BYPASS !== "virtual" && CONFIG.DIMMERS[dimmer_name].BYPASS !== "contactor") {
      print(scriptName, ":", "ERR: BYPASS must be either 'virtual' or 'contactor'");
      return;
    }
...
function onSwitchGetStatus(result, errCode, errMessage, data) {
  ...
  for (let dimmer_name in DIVERT.dimmers) {
    ...
    if (CONFIG.DIMMERS[dimmer_name].BYPASS === "contactor") {
    ...
    } else if (CONFIG.DIMMERS[dimmer_name].BYPASS === "virtual") {
  ...
  if (request.query) {
    request.query.split("&").forEach(function (param) {
      ...
      if (kv[0] === "debug") {
        ...
        print(scriptName, ":", "[HTTP] Setting PID setpoint to", kv[1]);
        CONFIG.PID.SETPOINT = parseInt(kv[1]);

      } if (kv[0] === "reset") {
        print(scriptName, ":", "[HTTP] Reset");
        PID.input = 0;
        PID.output = 0;
        PID.error = 0;
        PID.pTerm = 0;
