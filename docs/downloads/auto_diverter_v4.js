...
function callDimmer(ip, dimmer, cb) {
  const url = "http://" + ip + "/rpc/Light.Set?id=0&on=" + (dimmer.dutyCycle > 0 ? "true" : "false") + "&brightness=" + constrain(Math.round(dimmer.dutyCycle * 100), 0, 100) + "&transition_duration=0.5";
  if (CONFIG.DEBUG > 1)
...
function divert(gridVoltage, gridPower) {
  ...
  for (let ip in DIVERT.dimmers) {
    ...
    if (CONFIG.DEBUG > 0)
      print(scriptName, ":", "Dimmer", ip, "=>", dimmer.powerToDivert, "W");
  }
