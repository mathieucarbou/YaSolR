...
function callDimmer(ip, dimmer, cb) {
  const url = "http://" + ip + "/rpc/Light.Set?id=0&on=" + (dimmer.dutyCycle > 0 ? "true" : "false") + "&brightness=" + Math.max(0, Math.min(100, Math.round(dimmer.dutyCycle * 100))) + "&transition_duration=0.5";
  if (CONFIG.DEBUG > 1)
