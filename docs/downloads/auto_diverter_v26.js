...
function calculatePID(input) {
  ...
  if (CONFIG.PID.P_MODE === "error") {
    ...
    // clamp proportional term
    PID.pTerm = constrain(PID.pTerm, -PID.out_max, PID.out_max);
  }
  ...
  if (CONFIG.DEBUG > 0) {
    print(scriptName, ":", "[PID] Mode: ", PID.mode, ", Input:", input, "W, Error:", error, "W, pTerm:", PID.pTerm, "W, iTerm:", PID.iTerm, "W, dTerm:", PID.dTerm, "W, Output:", PID.output, "W");
  }
...
function divert(gridVoltage, gridPower) {
  ...
  // Reset PID terms if we are not routing for some reasons and we have some excess power
