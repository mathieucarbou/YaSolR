...
{  
  ...
  digitalWrite(physicalLoad_3_pin, !physicalLoadState[3]); // update the additional load state (inverse logic).      
  digitalWrite(physicalLoad_4_pin, !physicalLoadState[4]); // update the additional load state (inverse logic).      
  digitalWrite(physicalLoad_5_pin, !physicalLoadState[5]); // update the additional load state (inverse logic).      
  ...
  Serial.println();
  Serial.println("-------------------------------------");
  Serial.println("Sketch ID:      Mk2_multiLoad_CAT5_1.ino");
  Serial.println();
    ...
    {     
      ...
      if (triggerNeedsToBeArmed == true)
      {
        ...
        if(samplesDuringThisMainsCycle == 3) // should always exceed 20V (the min for trigger)
        {
          ...
          digitalWrite(physicalLoad_0_pin, !physicalLoadState[0]); // active high for SSR     
          digitalWrite(physicalLoad_1_pin, !physicalLoadState[1]); // active high for SSR     
          digitalWrite(physicalLoad_2_pin, !physicalLoadState[2]); // active high for SSR    
          digitalWrite(physicalLoad_3_pin, !physicalLoadState[3]); // active high for SSR     
          digitalWrite(physicalLoad_4_pin, !physicalLoadState[4]); // active high for SSR     
          digitalWrite(physicalLoad_5_pin, !physicalLoadState[5]); // active high for SSR     
