 ...
 * version of the Mk2 code, the system uses a single-threshold version of the 
 * anti-flicker algorithm which is well suited for multiple loads.  As the 'normal' 
 * mode is no longer required, the 'mode' port has been re-assigned for priority 
 * selection.   
 *
 ...
 *
 * As with all Mk2 PV Router sketches, the output stage is intended to be fed with an 
 * active-low signal.  If active-high control logic is to be used instead (e.g. for 
 * driving SSRs), an inversion will be required whenever the state of physical IO pin 
 * is assigned in the code.  This can be easily achieved by use of the '!' character.
 *
 * To drive an SSR rather than a triac, a greater voltage for the control signal is 
 * required.  A transistor stage can be used to switch the unregulated power supply 
 * to the SSR. 
 *
...
{  
  ...
  digitalWrite(physicalLoad_0_pin, physicalLoadState[0]); // update the local load's state (active low).      
  digitalWrite(physicalLoad_1_pin, physicalLoadState[1]); // update the additional load state (active low).      
  digitalWrite(physicalLoad_2_pin, physicalLoadState[2]); // update the additional load state (active low).      
  digitalWrite(physicalLoad_3_pin, physicalLoadState[3]); // update the additional load state (active low)).      
  digitalWrite(physicalLoad_4_pin, physicalLoadState[4]); // update the additional load state (active low).      
  digitalWrite(physicalLoad_5_pin, physicalLoadState[5]); // update the additional load state (active low).      
  ...
  Serial.println();
  Serial.println("-------------------------------------");
  Serial.println("Sketch ID:      Mk2_multiLoad_CAT5_2.ino");
  Serial.println();
  ...
  { 
    ...
    {     
      if (polarityOfLastSampleV != POSITIVE)
      {
        ...
        //
        long realPower_grid = sumP_grid / samplesDuringThisMainsCycle; // proportional to Watts
        long realPower_diverted = sumP_diverted / samplesDuringThisMainsCycle; // proportional to Watts
      ...
      if (triggerNeedsToBeArmed == true)
      {
        ...
        if(samplesDuringThisMainsCycle == 3) // should always exceed 20V (the min for trigger)
        {
          ...
          digitalWrite(physicalLoad_3_pin, physicalLoadState[3]); // active low for trigger    
          digitalWrite(physicalLoad_4_pin, physicalLoadState[4]); // active low for trigger    
          digitalWrite(physicalLoad_5_pin, physicalLoadState[5]); // active low for trigger    
...
void increaseLoadIfPossible()
{
  ...
  // then only if the energy level has not have fallen since the previous ON transition.  
...
//          energyAtLastOffTransition_long = 3600; // reset the 'opposite' mechanism. <-WRONG!
          energyAtLastOffTransition_long = capacityOfEnergyBucket_long; // reset the 'opposite' mechanism.
          activeLoadID = i;
          changed = true; 
        }
      }
...
void decreaseLoadIfPossible()
{
  ...
  // Only one load may operate freely at a time.  Other loads are prevented from 
  // switching until a sufficient period has elapsed since the last transition, and 
  // then only if the energy level has not have risen since the previous OFF transition.  
  // These measures allow a lower priority load to contribute if a higher priority
  // load is not having the desired effect, but not immediately.
  // 
  if (energyInBucket_long <= energyAtLastOnTransition_long)
 ...
 * the physical load 1 is set to have priority, the logical-to-physical association for
 * loads 0 and 1 are swapped.
 *
 * For this implementation, all loads are 'local' because the RF facility is not in use.  
 */
{
  for (int i = 0; i < noOfDumploads; i++)
  {
