/* Mk2_RFremoteLoad_1.ino
 *
 * This sketch is for diverting surplus PV power to a local dump load using a triac. 
 * A remote load is also supported, this being controlled using the on-board 
 * RFM12B module. A switch is also supported which allows either load to be 
 * selected as having thei higher priority.  If a local load is not provided, 
 * all surplus power is available for use by the remote load. 
 *
...
{  
  ...
  Serial.println();
  Serial.println("-------------------------------------");
  Serial.println("Sketch ID:      Mk2_withRemoteLoad_1.ino");
  Serial.println();
...
//            tx_data.dumpState = messageNumber & 1; // for test only
            send_rf_data();
 /*         
