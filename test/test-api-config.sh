...
assert "$(keys /api/config)" "admin_pwd,ap_mode_enable,debug_enable,pin_disp_scl,pin_disp_sda,disp_enable,disp_angle,disp_type,grid_freq,grid_pow_mqtt,ha_disco_enable,ha_disco_topic,hostname,jsy_enable,pin_jsy_rx,pin_jsy_tx,lights_enable,pin_lights_g,pin_lights_r,pin_lights_y,mqtt_enable,mqtt_port,mqtt_pub_itvl,mqtt_pwd,mqtt_secure,mqtt_server,mqtt_topic,mqtt_user,ntp_server,ntp_timezone,o1_ab_enable,o1_relay_enable,pin_o1_relay,o1_relay_type,o1_ad_enable,o1_enable,o1_dim_limit,pin_o1_dim,o1_time_stop,o1_time_start,o1_ds18_enable,o1_temp_stop,o1_temp_start,pin_o1_ds18,o1_days,o2_ab_enable,o2_relay_enable,pin_o2_relay,o2_relay_type,o2_ad_enable,o2_enable,o2_dim_limit,pin_o2_dim,o2_time_stop,o2_time_start,o2_ds18_enable,o2_temp_stop,o2_temp_start,pin_o2_ds18,o2_days,portal_timeout,relay1_enable,pin_relay1,relay1_load,relay1_type,relay2_enable,pin_relay2,relay2_load,relay2_type,ds18_sys_enable,pin_ds18,wifi_pwd,wifi_ssid,wifi_timeout,zcd_enable,pin_zcd"
assert "$(value /api/config 'admin_pwd')" ""
assert "$(value /api/config 'ap_mode_enable')" "false"
assert "$(value /api/config 'debug_enable')" "true"
assert "$(value /api/config 'pin_disp_scl')" "22"
assert "$(value /api/config 'pin_disp_sda')" "21"
...
assert "$(value /api/config 'o1_enable')" "true"
...
assert "$(value /api/config 'o2_enable')" "true"
