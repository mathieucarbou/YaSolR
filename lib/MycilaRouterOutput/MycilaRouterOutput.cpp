...
bool Mycila::RouterOutput::isMeterConnected() const { return _pzem->isConnected(); }
float Mycila::RouterOutput::getEnergy() const { return _pzem->getEnergy(); }
float Mycila::RouterOutput::getCurrent() const { return _dimmer->isOn() ? _pzem->getCurrent() : 0; }
float Mycila::RouterOutput::getActivePower() const { return _dimmer->isOn() ? _pzem->getPower() : 0; }
float Mycila::RouterOutput::getApparentPower() const { return getCurrent() * Mycila::Grid.getVoltage(); }
float Mycila::RouterOutput::getPowerFactor() const {
  const float va = getApparentPower();
  return va == 0 ? 0 : getActivePower() / va;
}
float Mycila::RouterOutput::getVoltage() const { return _dimmer->isOn() ? _pzem->getVoltage() : 0; }
float Mycila::RouterOutput::getTHDi() const {
  const float pf = getPowerFactor();
  if (pf == 0)
    return 0;
...
float Mycila::RouterOutput::getResistance() const {
  const float i = getCurrent();
  return i == 0 ? 0 : getVoltage() / i;
}
