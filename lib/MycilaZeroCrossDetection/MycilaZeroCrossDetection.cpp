...
void Mycila::ZeroCrossDetectionClass::begin() {
  if (_enabled)
    return;

  if (!ZCDConfig.isEnabled())
    return;

  int32_t pin = ZCDConfig.getPin();
  ...
  Logger.debug(TAG, "- Grid Frequency: %u Hz", ZCDConfig.getGridFrequency());
...
namespace Mycila {
  ...
  ZeroCrossDetectionConfigClass ZCDConfig;
} // namespace Mycila
