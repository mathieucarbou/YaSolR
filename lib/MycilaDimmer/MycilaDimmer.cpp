...
void Mycila::TriacDimmer::begin() {
  if (_enabled)
    return;

  if (!GPIO_IS_VALID_OUTPUT_GPIO(_pin)) {
    LOGE(TAG, "Disable Dimmer: Invalid pin: %" PRId8, _pin);
    return;
  }

  LOGI(TAG, "Enable Dimmer on pin %" PRId8 " with semi-period %" PRIu16 " us", _pin, _semiPeriod);
...
void Mycila::TriacDimmer::_applyDutyCycle(float mappedDutyCycle) {
  if (mappedDutyCycle == 0 || _semiPeriod == 0) {
    _delay = UINT16_MAX;
...
uint16_t Mycila::TriacDimmer::_lookupPhaseDelay(float dutyCycle) {
...
void ARDUINO_ISR_ATTR Mycila::TriacDimmer::onZeroCross(int16_t delayUntilZero, void* arg) {
